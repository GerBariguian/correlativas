...
export default function JointPlanPanel({ user, career, data, people, friends, nameOf, titleOf, planId, onPlanChange, onRetry, onAdd, onCompare, showCreate, setShowCreate, onCreated }) {
  ...
  return <section className="joint-plan planning-plan">
    ...
    {state === 'ready' && !plans.length && <div className="planning-empty"><h2>Todavía no tenés planes conjuntos</h2><p>Creá un plan e invitá a tus amigos para organizar materias juntos.</p><button className="planning-primary" onClick={() => setShowCreate(true)}>Crear un plan</button></div>}
    ...
    {plan && <>
      <header className="planning-plan-head">
        <div className="planning-section-head"><h2>{titleOf(plan)}</h2>{member && <details className="planning-options"><summary>Opciones del plan</summary><div>
          {editing && <><button onClick={() => { setNewName(titleOf(plan)); setAction('rename') }}>Renombrar</button><button onClick={() => setAction('invite')}>Agregar participante</button></>}
